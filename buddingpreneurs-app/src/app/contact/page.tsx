            ...
            {navLinks.map((link) => (
              <a
                ...
                className={`text-[11px] xl:text-xs font-semibold transition-colors ${
                  link.name === 'Contact' ? "text-[#C9540A] border-b-2 border-[#C9540A] pb-1 font-bold" : "text-[#0f172a] hover:text-slate-500"
                }`}
              >
                {link.name}
              </a>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-6 items-center text-center mt-8">
              {navLinks.map((link) => (
                <a key={link.name} href={link.path} onClick={() => setIsMobileMenuOpen(false)} className={`text-lg font-semibold transition-colors ${link.name === 'Contact' ? 'text-[#C9540A]' : 'text-[#0f172a] hover:text-[#C9540A]'}`}>
                  {link.name}
                </a>
              ))}
            </div>
